...
//   const model = item.series || item.brand || item.model || item.product_code_name || item.product_code || item.categoryName || "-";
...
// };
export const normalizeOrderProduct = (item = {}) => {
  const productName = item.product_name || item.product || item.name || "Unnamed Product";
  ...
  return {
    ...
    // readyStock: Number(item.ready_stock || 0),
...
export const orderItemProductSmartSelectConfig = {
  ...
  apiUrl: "",
  check: "product_name",
  list: "product_id,product_name,sku,unit,mrp,sale_price,tax_rate",
  preload: true,
  cache: true,
  showRecent: true,
  multi: false,
  ...
  normalized.sku,
  normalized.unit,
  normalized.salePrice ? `₹${normalized.salePrice}` : "",
    ].filter(Boolean).join(" \u2022 ");
  },
...
export const productSelectConfig = {
  type: "product",
  source: "products",
  list: "product_id,product_name,unit,sale_price,tax_rate,discount",
  check: "product_name",
  placeholder: "Select Product",
  preload: true,
  cache: true,

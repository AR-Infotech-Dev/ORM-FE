...
//         id: item.item_id || `${item.order_id || "order"}-${index}`,
//         item_id: item.order_item_id || null,
...
const mapApiOrderItems = (items = []) => {
    ...
    return items.map((item, index) => ({
        ...
        product_id: item.product_id || null,
        product: item.product || item.product_name_snapshot || item.product_name || null,
        qty: Number(item.qty || 0),
        unit: item.unit || "",
        unitRate: Number(item.rate || item.sale_price || 0),
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            ...
            try {
                ...
                const apiItems = orderData?.items || orderData?.order_items || [];
                setInitialOrderItems(mapApiOrderItems(apiItems));
        ...
        try {
            ...
            const payload = {
                order: {
                    ...
                    tax_amount: summary.gstAmount || 0,
                    total_amount: summary.grandTotal || 0,
                    order_status: statusOverride || formData.order_status,
                    status: "active",
                },
            ...
            if (res.success) {
                toast.success(res?.message || `Order ${mode === "create" ? "created" : "updated"} successfully`);
                setFormData(ordersModuleSchema.form.initialValues);
                setInitialOrderItems([]);
                onClose();
                onAfterSave?.();

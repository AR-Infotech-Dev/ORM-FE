...
import { orderItemProductSmartSelectConfig, productSelectConfig, } from "@modules/orders/shared/utils/orderProductSmartSelect.config";
    ...
    return (
        <div className={`items-center gap-2 px-2.5 py-2 transition hover:bg-orange-50/30 ${className}`}>
            ...
            <SmartSelectInput
                ...
                field={{ name: `product_${index}` }}
                value={item.product_id || ""}
                config={productSelectConfig}
                onSelect={(productId) => {
                    handleFieldChange(item.id, "product_id", productId);
                }}
                onObjectSelect={(option) => {
            ...
            <TextInput
                ...
                min="0"
                value={item.unit}
                onChange={(value) => handleFieldChange(item.id, "unit", value)}
            />
            ...
            <TextInput
                ...
                placeholder="%"
                value={item.discount}
                onChange={(value) => handleFieldChange(item.id, "discount", value)}
            />

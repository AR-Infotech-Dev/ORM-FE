...
const rowGridClass = "grid grid-cols-[28px_minmax(160px,1.8fr)_minmax(96px,1fr)_58px_58px_58px_58px_96px_34px]";

const OrderItems = ({ defaultItems = [], onItemsChange }) => {
  const [items, setItems] = useState(() => normalizeRows(defaultItems));
  ...
  const handleFieldChange = (rowId, field, value) => {
    const numericFields = ["qty", "unitRate", "discount", "gst"];
    updateItems(
      items.map((item) =>
        item.id === rowId
      ...
      items.map((item) =>
        item.id === rowId
          ? {
            ...
            // productCode: product.product_code || product.productCode || product.sku || null,
            unit: product.unit ?? "",
            unitRate: Number(product.sale_price) || 0,
            gst: Number(product.tax_rate) || 0,
            discount: Number(product.discount) || 0,
            // readyStock: Number(product.ready_stock) || 0,
            // pendingStock: Number(product.pending_stock) || 0,
            qty: item.qty || 0,
          }
          : item
      )

...
function OrderForm({ isOpen, onClose, selectedOrder, onAfterSave, menu_id }) {
  ...
  // const orderSummary = useMemo(() => buildOrderSummary(orderItems, formData), [orderItems]);
  const orderSummary = useMemo(
    () => buildOrderSummary(orderItems, formData),
    [orderItems, formData]
  );
  ...
  // const isDraft = formData?.order_status === "draft";
const isEdit = Boolean(selectedOrder?.order_id);
const originalStatus = selectedOrder?.order_status;

const isReadOnly = isEdit && originalStatus !== "draft";
const showActions = !isEdit || originalStatus === "draft";
          ...
          <div className="grid h-full min-h-0 grid-cols-1 gap-1 overflow-hidden p-0 lg:grid-cols-[280px_minmax(0,1fr)]">
            ...
            <div className="grid min-h-0 min-w-0 grid-rows-[minmax(0,1fr)_auto] gap-0 overflow-hidden">
              <div className="min-h-0 overflow-hidden">
                <OrderItems defaultItems={initialOrderItems} onItemsChange={setOrderItems} readOnly={isReadOnly} />
              </div>

              <section className="shrink-0 border border-slate-100 bg-white shadow-sm">
                <OrderSummary layout="horizontal" {...orderSummary} gstPercentage={18} />
              </section>
            </div>
          </div>
        </div>

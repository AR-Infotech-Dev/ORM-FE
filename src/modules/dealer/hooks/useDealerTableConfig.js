...
import { DealersFallbackColumns, DealersModuleSchema } from "../data/module.schema";

export const useDealersTableConfig = ({ resolvedMenuID, filterState }) => {
    ...
    const columnOptions = {
        skipFields: DealersModuleSchema.skipFields,
        columnMappings: DealersModuleSchema.columnMappings,
        tableCellConfig: DealersModuleSchema.tableCellConfig,
    };

    const resolvedColumns = useMemo(
        () => buildTableColumnsFromStructure(fields, DealersFallbackColumns, columnOptions),
        [fields]
    );

    const defaultVisibleColumnKeys = useMemo(
        () => DealersFallbackColumns.map((column) => column.key),
        []
    );
    ...
    const resolvedFilterFields = useMemo(() =>
        buildFilterFieldsFromStructure(
            fields,
            DealersModuleSchema.defaultColumns.map((key) => ({
                label:
                    DealersFallbackColumns.find((column) => column.key === key)?.label || key,
                value: key,
                type: "text",
            })),
            columnOptions
